...
namespace JKFrame
{
    ...
    public class UISystem : MonoBehaviour
    {
        ...
        private static Dictionary<string, UIWindowData> UIWindowDataDic => JKFrameRoot.toolsSetting.uiWindowDataDic;
        ...
        public static void AddUIWindowData(string windowKey, UIWindowData windowData, bool instantiateAtOnce = false)
        {
            if (UIWindowDataDic.TryAdd(windowKey, windowData))
            {
                if (instantiateAtOnce)
                {
                    ...
                    {
                        UI_WindowBase window = ResourceManager.InstantiateGameObject<UI_WindowBase>(windowData.assetPath, UILayers[windowData.layerNum].root, windowKey);
        ...
        private static UI_WindowBase Show(UIWindowData windowData, string windowKey, int layer = -1)
        {
            ...
            {
                UI_WindowBase window = ResourceManager.InstantiateGameObject<UI_WindowBase>(windowData.assetPath, UILayers[layerNum].root, windowKey);
                windowData.instance = window;
                window.Init();
