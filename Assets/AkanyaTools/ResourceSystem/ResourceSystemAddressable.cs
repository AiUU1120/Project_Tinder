...
namespace FrameTools.ResourceSystem
{
    public static class ResourceManager
    {
        ...
        /// 卸载普通对象，这里是使用对象池的方式
        /// </summary>
        public static void PushObjectInPool(global::System.Object obj)
        {
        ...
        /// <param name="maxCapacity">容量限制，超出时会销毁而不是进入对象池，-1代表无限</param>
        public static void InitObjectPool(global::System.Type type, int maxCapacity = -1)
        {
        ...
        public static void InitGameObjectPoolForKeyName(string keyName, int maxCapacity = -1, string assetName = null, int defaultQuantity = 0)
        {
            ...
            {
                var gameObjects = new GameObject[defaultQuantity];
                for (var i = 0; i < defaultQuantity; i++)
                {
        ...
        private static void OnInstantiateGameObjectAsyncCompleted<T>(AsyncOperationHandle<GameObject> handle, Action<T> callback, string gameObjectName, bool autoRelease = true)
        ...
        /// <param name="parent">父物体</param>
        /// <param name="autoRelease">物体销毁时，会自动去调用一次Addressables.Release</param>
        public static T InstantiateGameObject<T>(Transform parent, string keyName, bool autoRelease = true) where T : UnityEngine.Component
        {
            var go = InstantiateGameObject(parent, keyName, autoRelease);
            return !go.IsNull() ? go.GetComponent<T>() : null;
        }
        ...
        /// <param name="parent">父物体</param>
        /// <param name="autoRelease">物体销毁时，会自动去调用一次Addressables.Release</param>
        public static T InstantiateGameObject<T>(string assetName, Transform parent = null, string keyName = null, bool autoRelease = true) where T : UnityEngine.Component
        {
            var go = InstantiateGameObject(assetName, parent, keyName, autoRelease);
            return !go.IsNull() ? go.GetComponent<T>() : null;
        }
        ...
        public static void InstantiateGameObjectAsync<T>(string assetName, Action<T> callback = null, Transform parent = null, string keyName = null, bool autoRelease = true)
            where T : UnityEngine.Component
        {
            var go = PoolSystem.GetGameObject(keyName ?? assetName, parent);
            // 对象池中有
            ...
            // 不通过缓存池
            Addressables.InstantiateAsync(assetName, parent).Completed += (handle) => { OnInstantiateGameObjectAsyncCompleted<T>(handle, callback, keyName ?? assetName, autoRelease); };
        }
        ...
        /// </summary>
        /// <param name="assetName">AB资源名称</param>
        public static T LoadAsset<T>(string assetName) where T : UnityEngine.Object => Addressables.LoadAssetAsync<T>(assetName).WaitForCompletion();
        ...
        {
            Addressables.LoadAssetAsync<T>(assetName).Completed += (handle) => { OnLoadAssetAsyncCompleted<T>(handle, callback); };
        }
        ...
        /// 销毁游戏物体并释放资源
        /// </summary>
        public static bool UnloadInstance(GameObject obj) => Addressables.ReleaseInstance(obj);
