...
[视频演示](https://aiuu.moe/log/679/)

## Playable Kami

基于 Playable 的动画播放框架，可脱离 Animation Controller 使用。动态生成与卸载 Playable 节点。支持动画混合。

<img src="https://s2.loli.net/2024/09/15/4qp6J8umLngfjPG.gif" width="90%">

<img src="https://s2.loli.net/2025/04/05/4m21DlNKBYjQxn7.gif" width="50%">

## Skill Master

基于 UI Toolkit 的技能编辑器，支持动画、特效、音效、判定检测与自定义事件的可视化编辑与运行时播放。

### 总览

编辑器内采用模拟预览的方式对技能进行编辑，封装动画、判定检测、特效、音效的数据供运行时使用。

<img src="https://s2.loli.net/2025/04/05/uiw8nM7gEv9V6Bq.gif" width="90%">
...
- Skill Brain：技能最上层，技能的消耗来源，技能之间的共享数据
- Skill Config：
  - Skill Clips：全部的技能片段
  - Skill Behavior：技能逻辑的掌控者，对Skill Clips进行状态管理
- Skill Behavior：
  - 监听Skill Clip中的自定义事件，以及决定播放哪一个Skill Clip，对技能进行状态管理
  - 可以在编辑器所有事件前、后做特殊化处理，可以拦截事件的发生（比如变身后所有攻击范围加大50%、变身后技能音效改变、致盲后攻击判定无效），也就是对配置数据进行二次加工来完成特殊逻辑
- Skill Clip：技能编辑器的产物，播放动画、音效、特效以及向上发出事件
- Skill Player：技能播放器，播放的是Skill Clip，将Skill Clip中的事件向上发送给Skill Behavior

### 技能控制层级

PlayerController => SkillBrain => SkillBehaviour
